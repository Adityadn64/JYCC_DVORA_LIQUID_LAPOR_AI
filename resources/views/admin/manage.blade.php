...
<div class="space-y-8">
    @php
        ...
        foreach($admins as $admin) {
            $totalPendingAdmin += $admin->status === \App\Enums\AdminStatusEnum::Pending ? 1: 0;
        }
    @endphp
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        ...
        <div class="flex-shrink-0 flex gap-2">
            <a href="{{ route('admin.manage.request') }}" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 cursor-pointer">
                ({{ $totalPendingAdmin }}) Lihat Permintaan Admin yang Tertunda
            </a>
        </div>
    </div>
    ...
    <div class="bg-white shadow-lg rounded-xl border overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @php
                    function compareAdminsByRole($adminA, $adminB) {
                        $roleOrder = [
                            App\Enums\RoleAdministratorEnum::SystemAdmin->value => 1,
                            App\Enums\RoleAdministratorEnum::BaseAdmin->value => 2,
                        ];

                        $orderA = $roleOrder[$adminA->role->value] ?? 99;
                        $orderB = $roleOrder[$adminB->role->value] ?? 99;
                    ...
                    foreach($admins as $admin) {
                        if ($admin->status === \App\Enums\AdminStatusEnum::Pending) {
                            continue;
                        }
                    ...
                    usort($filteredAdmins, 'compareAdminsByRole');
                @endphp
