...
const CLIENT_URLS = JSON.parse(process.env.CLIENT_URLS || `['http://localhost:3000']`);
const DEFAULT_SERVER_API_URL = process.env.DEFAULT_SERVER_API_URL || "http://localhost:3001";
const SERVER_API_URLS = JSON.parse(process.env.SERVER_API_URLS || `[${DEFAULT_SERVER_API_URL}]`);
...
// ================================
const laravelAPICSRF = axios.create({
...
const vigenereCipher = (input, key, mode) => {
...
const decodePayload = (encodedPayload, key) => {
    try {
...
const proccessResponseData = (response) => {
    try {
...
laravelAPI.interceptors.request.use((config) => {
    const token = app.get('request')?.session?.auth_token ||
...
laravelAPI.interceptors.response.use((response) => response, (error) => {
...
app.post('/api/regencies', async (req, res) => {
    try {
...
// CSRF TOKEN ENDPOINT
// ================================
app.get('/api/csrf-cookie', async (_req, res) => {
    try {
...
app.post('/api/report/create', upload.any(), async (req, res) => {
    try {
...
app.post('/api/reports/track', async (req, res) => {
    try {
...
app.post('/api/report/:id/track', async (req, res) => {
    try {
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const parsedResponse = proccessResponseData(laravelResponse);
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const parsedResponse = proccessResponseData(response);
...
// ================================
// ADMIN DASHBOARD
app.post('/api/admin/dashboard', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/analytics/export-reports', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/profile/update-info', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/profile/update-password', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/profile/request-email-change', authMiddleware, async (req, res) => {
    try {
...
// ADMIN MANAGE
app.post('/api/admin/manage', authMiddleware, async (req, res) => {
    try {
...
// ADMIN MANAGE UPDATE
app.put('/api/admin/manage/:id', authMiddleware, async (req, res) => {
    try {
...
// ADMIN MANAGE DELETE
app.delete('/api/admin/manage/:id', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/manage/:id/toggle-status', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/manage/:id/reset-password', authMiddleware, async (req, res) => {
    try {
...
// ADMIN PERFORMANCE
app.post('/api/admin/performance', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/admin/performance/export', authMiddleware, async (req, res) => {
    try {
