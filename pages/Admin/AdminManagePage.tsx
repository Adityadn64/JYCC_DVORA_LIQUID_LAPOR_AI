...
import { Skeleton } from "../../components/SkeletonLoading/Admin/SkeletonLoadingAdminPage";
      ...
      <div className="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
        <div className="mt-3 text-center">
          ...
          <div className="items-center px-4 py-3">
            <button
              onClick={onConfirm}
              className="px-4 py-2 bg-blue-600 text-white text-base font-medium rounded-md w-auto shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              Ya, Lanjutkan
            </button>
            <button
              onClick={onClose}
              className="px-4 py-2 bg-gray-200 text-gray-800 text-base font-medium rounded-md w-auto shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-300 ml-2"
            >
              Batal
            </button>
          </div>
...
const ActivityDrawer: React.FC<ActivityDrawerProps> = ({
    ...
    if (isOpen && adminId) {
      const fetchActivity = async () => {
        ...
        try {
          ...
          setError(
            (await decodeErrorResponse(err)) || "Gagal memuat aktivitas"
          );
        } finally {
          setLoading(false);
        }
  ...
  return (
    <div
      className={`fixed inset-y-0 right-0 z-50 w-full max-w-lg bg-white shadow-xl transition-transform duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="p-6 overflow-y-auto h-full">
        {loading && (
          <div className="text-center py-10">Memuat data aktivitas...</div>
        )}
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-4">
              <img
                ...
                src={
                  activityData.admin.profile_picture_path
                    ? `/storage/${activityData.admin.profile_picture_path}`
                    : `https://ui-avatars.com/api/?name=${encodeURIComponent(
                        activityData.admin.full_name
                      )}`
                }
                alt={activityData.admin.full_name}
              />
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {activityData.admin.service_profile?.full_name ??
                    (activityData.admin.role === "system_admin"
                      ? "System Admin"
                      : "N/A")}
                </p>
              </div>
            ...
            <dl className="space-y-2">
              <div className="flex justify-between">
                ...
                <dd className="text-sm font-medium">
                  {activityData.admin.email}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-sm text-gray-500">Telepon</dt>
                <dd className="text-sm font-medium">
                  {activityData.admin.phone}
                </dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-sm text-gray-500">Bergabung</dt>
                <dd className="text-sm font-medium">
                  {new Date(activityData.admin.created_at).toLocaleDateString(
                    "id-ID",
            ...
            <div>
              <h5 className="text-md font-semibold text-gray-800 mb-3">
                10 Laporan Terakhir Ditangani
              </h5>
              <ul className="divide-y divide-gray-200 border rounded-md">
                {activityData.recent_reports.length > 0 ? (
                  activityData.recent_reports.map((report) => {
                    const status =
                      report.statuses[report.statuses.length - 1] || "unknown";
                    const updated = new Date(report.updated_at).toLocaleString(
                      "id-ID",
                      ...
                      <li key={report.id} className="px-4 py-3">
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-500">
                            #{report.id} - Status: {status} - {updated}
                          </p>
                        </a>
                      </li>
                    );
...
export default function AdminManagePage({ csrfLoading }: CsrfLoadingProps) {
  ...
  const fetchAdmins = useCallback(
    async (page = 1, currentFilters = filters, mode = viewMode) => {
      setLoading(true);
      setError(null);
      try {
        const params = {
        ...
        setError(
          (await decodeErrorResponse(err)) || "Gagal memuat data administrator"
        );
      } finally {
        setLoading(false);
      }
  ...
  const renderAdminTable = () => {
    ...
    if (error) {
      return (
        <tr>
          <td colSpan={6}>{errorMessage(error)}</td>
        </tr>
      );
    }
    if (admins.length === 0) {
    ...
    return admins.map((admin) => (
      ...
      >
              ...
              <img
                ...
                src={
                  admin.profile_picture_path
                    ? `/storage/${admin.profile_picture_path}`
                    : `https://ui-avatars.com/api/?name=${encodeURIComponent(
                        admin.full_name
                      )}`
                }
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <div className="text-sm font-medium text-gray-900">
            {admin.role === "system_admin" ? "System Admin" : "Base Admin"}
          </div>
          <div className="text-sm text-gray-500">
            {admin.service_profile?.full_name ??
              (admin.role === "system_admin" ? "System-wide" : "N/A")}
          </div>
        </td>
        <td className="px-6 py-4 whitespace-nowrap">
          {admin.status === "active" && (
        ...
        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
          {formatRelativeTime(admin.updated_at)}
        </td>
        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
          {viewMode === "manage" ? (
            <>
              <button
                onClick={() => setActivityAdminId(admin.id)}
              ...
              <button
                onClick={() =>
                  setConfirmModalProps({
                    ...
                    onConfirm: () =>
                      handleAction(
                        adminManageService.toggleStatus(admin.id),
                        "Status admin berhasil diubah."
                      ),
                  })
                }
              ...
              <button
                onClick={() =>
                  setConfirmModalProps({
                    ...
                    message: `Kirim link reset password ke '${admin.full_name}'?`,
                    onConfirm: () =>
                      handleAction(
                        adminManageService.sendPasswordReset(admin.id),
              ...
              <button
                onClick={() =>
                  setConfirmModalProps({
                    ...
                    message: `Anda yakin ingin menerima dan mengaktifkan admin '${admin.full_name}'?`,
                    onConfirm: () =>
                      handleAction(
                        adminManageService.accept(admin.id),
                        "Admin berhasil diterima dan diaktifkan."
                      ),
                  })
                }
              ...
              <button
                onClick={() =>
                  setConfirmModalProps({
                    ...
                    message: `Anda yakin ingin menolak dan menghapus pendaftaran '${admin.full_name}'?`,
                    onConfirm: () =>
                      handleAction(
                        adminManageService.reject(admin.id),
                        "Pendaftaran admin berhasil ditolak."
                      ),
                  })
                }
                className="text-red-600 hover:text-red-900"
  ...
  return (
    <div>
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              {viewMode === "manage"
                ...
                : "Permintaan Admin Tertunda"}
            </h1>
            <p className="mt-2 text-gray-600">
              {viewMode === "manage"
                ? "Buat dan kelola semua akun administrator sistem."
                : `Total ${pendingCount} admin menunggu persetujuan.`}
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              >
                ...
                {filterOptions.roles.map((r) => (
                  <option key={r.value} value={r.value}>
                    {r.value.charAt(0).toUpperCase() +
                      r.value.slice(1).split("_").join(" ")}
                  </option>
                ))}
              </select>
              {viewMode === "manage" && (
                ...
                >
                  ...
                  {filterOptions.statuses
                    .filter((s) => s.value.toLowerCase() !== "pending")
                    .map((s) => (
              ...
              >
                ...
                {filterOptions.services.map((s) => (
                  <option key={s.code.value} value={s.code.value}>
                    {s.full_name}
                  </option>
                ))}
              </select>
              ...
              {paginationItems.map((item, index) =>
                item === DOTS ? (
                  ...
                  <button
                    ...
                    aria-current={
                      item === paginationInfo.current_page ? "page" : undefined
                    }
                    className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                      item === paginationInfo.current_page
      ...
      />
      <ActivityDrawer
        isOpen={!!activityAdminId}
        onClose={() => setActivityAdminId(null)}
