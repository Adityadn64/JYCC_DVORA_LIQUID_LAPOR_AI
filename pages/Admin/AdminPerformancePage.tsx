...
import {
  ...
  decodeErrorResponse,
} from "../../services/api"; // Asumsi service API Anda
import { Skeleton } from "../../components/SkeletonLoading/Admin/SkeletonLoadingAdminPage"; // Asumsi komponen Skeleton Anda
...
export default function AdminPerformancePage({
  ...
  const fetchPerformance = async () => {
    ...
    try {
      ...
      setError(
        (await decodeErrorResponse(err)) || "Gagal memuat data performa"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleReset = () => {
    ...
    setScopeValue("");
    setDateStart(
      toYmd(new Date(new Date().setDate(new Date().getDate() - 30)))
    );
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select
                ...
                disabled={scopeType === "all" || loading}
                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm p-2 border outline-none focus:ring-1 focus:ring-blue-500 disabled:bg-gray-100"
              >
                <option value="">
                  {scopeType === "all" ? "(Tidak Perlu)" : "Semua"}
                </option>
                {/* PERBAIKAN #1: Menggunakan `opt.id` dan `opt.full_name` untuk filter options */}
                {scopeType === "dinas" &&
                  performanceData?.filterOptions.services.map((opt) => (
                    <option key={opt.id} value={opt.id}>
                      {opt.full_name}
                    </option>
                  ))}
                {scopeType === "admin" &&
                  performanceData?.filterOptions.admins.map((opt) => (
                    <option key={opt.id} value={opt.id}>
                      {opt.full_name}
                    </option>
                  ))}
                {scopeType === "district" &&
                  performanceData?.filterOptions.districts.map((opt) => (
          ...
          <div className="mt-6 flex items-center justify-end gap-x-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="rounded-md bg-blue-600 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 disabled:bg-blue-300"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {loading || !performanceData ? (
          ...
          <>
            <KpiCard
              title="Total Laporan"
              value={performanceData.kpiCards.total.toLocaleString()}
              color="text-blue-600"
            />
            <KpiCard
              title="Laporan Hari Ini"
              value={performanceData.kpiCards.today.toLocaleString()}
              color="text-blue-500"
            />
            <KpiCard
              title="Penyelesaian Tepat Waktu"
              value={`${performanceData.kpiCards.sla_percent.toFixed(1)}%`}
            ...
            <KpiCard
              ...
              value={
                performanceData.kpiCards.avg_hours
                  ? `${parseFloat(performanceData.kpiCards.avg_hours).toFixed(
                      1
                    )} Jam`
                  : "N/A"
              }
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                performanceData?.dinasPerformance.map((dinas) => {
                  const slaPercent =
                    dinas.total_selesai > 0
                      ? 100 * (1 - dinas.sla_breaches / dinas.total_selesai)
                      : 100;
                  const slaColor =
                    slaPercent < 80 ? "text-red-600" : "text-green-600";
                  return (
                    <tr key={dinas.full_name}>
                      ...
                      <td className="px-4 py-2">
                        {dinas.avg_hours
                          ? parseFloat(dinas.avg_hours).toFixed(1)
                          : "N/A"}
                      </td>
                      <td className={`px-4 py-2 font-medium ${slaColor}`}>
              ...
              !Array.isArray(performanceData?.adminPerformance) ? (
                <tr>
                  <td
                    ...
                    className="px-4 py-4 text-center text-gray-500"
                  >
                    Format data kinerja admin tidak valid.
                  </td>
                </tr>
              ) : (
                performanceData?.adminPerformance.map(
                  (
                    admin: any // Gunakan `any` karena data asli tidak diketahui
                  ) => (
                    <tr key={admin.full_name}>
                      ...
                      <td className="px-4 py-2">
                        {admin.avg_hours
                          ? parseFloat(admin.avg_hours).toFixed(1)
                          : "N/A"}
                      </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-red-50 p-6 rounded-xl shadow border border-red-200">
          <h3 className="text-lg font-semibold text-red-800 mb-4">
            Laporan Mendesak (Lewat SLA & Aktif)
          </h3>
          <div className="overflow-x-auto max-h-96">
            <ul className="divide-y divide-red-200">
              {loading ? (
                ...
                <li className="py-4 text-center text-red-700">
                  👍 Tidak ada laporan aktif yang melewati SLA.
                </li>
              ) : (
                performanceData?.slaBreaches.map((report) => (
                  <li key={report.id} className="py-3">
                    <a
                      ...
                      className="block hover:bg-red-100 p-2 rounded-md"
                    >
                      ...
                      <p className="text-xs text-red-700">
                        Dibuat: {formatDateRelative(report.created_at)} |
                        Penanggung: {report.assignee?.full_name ?? "N/A"}
                      </p>
                    </a>
                  </li>
                ))
